...
def before_2000(hashed_list)
  ...
  hashed_list.each do |key, value|
    if value >= 2000
      new_hashed_list << [key, value]
    end
  new_hashed_list
end
...
def update_list(hashed_list)
  hashed_list.each {|key, value|
    hashed_list[value] = value - 3
  }
  hashed_list
end
puts update_list(extinct_animals).each { |key, value| puts "#{key} - #{value}." }
...
def check_if_endagered(check_animal, hashed_list)
  counter = 0
  
  while counter < hashed_list
